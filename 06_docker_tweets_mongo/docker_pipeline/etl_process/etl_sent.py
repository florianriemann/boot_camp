...
tweets = db_mongo.tweets_gazprom
...
db_pg = create_engine('postgresql://postgres:12345678@postgres_db:5432/postgres', echo=True)   # host look-up with sudo docker ps -a
...
def transform(extracted_tweets):
    ...
    for tweet in extracted_tweets:
        #print("####################################", tweet)
        #print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", type(tweet))
        #print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", tweet["text"])
        sentiment = analyzer.polarity_scores(tweet["text"])['compound']                                                  # later on you will calculate a sentiment
...
def load(transformed_tweets):
    ...
    for tweet in transformed_tweets:
        ...
        db_pg.execute(insert_query, (tweet['text'], tweet['sentiment']))
